    ...
    if (ESP_BT.available()) 
    {
        ...
        {   
            ...
            else
            {
                ...
                // Verificar guardada desde BT app ////////
                File names = SD.open("/names.txt", FILE_APPEND);        //tenia FILE_WRITE 
                if (names) 
                {
                    String item;
                    // Get name form array to string!
                    // msg_rcv[loop_var] //VER LIMITES!
                    // caca
                    for(int k=9; k<=loop_var; k++)
                    {
                        item += String(msg_rcv[k]);
                        //Serial.println("File Name from BT:"+item);
                    }                    
                    names.println(item);                            //add the newly added pattern to the names file
                    names.close();
                    #ifdef VERBOSE_SAVE_BT
                        Serial.println("Save file with name:" + String(item));
                    #endif
                } 
                else 
                {
                    #ifdef VERBOSE_SAVE_BT
                        Serial.println("Can't open name.txt to save from BT req");
                    #endif
                }

                int otra_i = 0;     
                while (SD.exists("/" + String(otra_i) + ".txt")) 
                ...
                file_to_save_to = otra_i;        // me guardo cosillas
                /////////////////////////////////////  
