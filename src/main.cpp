...
// rancio que tenía que terminaba funcionando mal
// Acomodar cosillas en carpetas en la SD
// Poner un boton de save en el converter
// Temporizar desde el arranque si no se hace nada (ni conexion wifi ni BT)
// y ponerse automáticamente a scrollear imágenes
// Quizás un explorador croto para copiar, descargar, eliminar files
// Quiero arreglar la pintada, sólida, sin el bucle (o hacer el mismo bucle en un lado y otro)
// para que funcione más mejor y no sature de peticiones
...
// Arreglar la app BT para que no me dé falsos negativos cuando grabo archivps:
...
// Se lo dejo al Guga del futuro
  ...
  { 
    ...
    delay(2500); //tempo para a tarea iniciar
...
// ¿? y así para arriba...    
    //BT_init();  //- Si no lo hago así se me cuelga cuando todo junto quiere ocurrir!
    // Ver condiciones iniciales. Coincidir con lo que muestran los AP y la App
    FIND_CURSOR = true;
    canvas_size = 16;
    color_to_deposit = 0;
...
// Poner numero de archivo (nombre de archivo) en variable global "file_to_save_to"
void save_large()
{
...
void loop() 
{
  ...
  { 
    ...
    else if(live == false)
    {
      ...
      //   loopIndex = 0;
      // } //la comprobación de existencia la hace el método ahora, así lo puedo reciclar
